...
function do_enter_individual_score() {
	...
	$result = lmt_query('SELECT id, name, attendance, score_individual FROM individuals WHERE name="'
	...
	lmt_query('UPDATE individuals SET score_individual="'
...
function do_enter_clarified_score() {
	...
	$row = lmt_query('SELECT name, score_individual FROM individuals WHERE id="'
