<?php
/*
 * LMT/Backstage/Scoring/Team_Long.php
...
HEREDOC;
	...
	echo <<<HEREDOC
      <h1>Team Round (Long) Score Entry</h1>
      $err$redirAlert$msg
...
function do_enter_team_score() {
	...
	if (!is_null($row['score_team_long'])) {
		$msg = 'A score of ' . htmlentities($row['score_team_long'])
			. ' has already been entered for ' . htmlentities($row['name']);
		if ($row['score_team_long'] != $_POST['score'])
			$msg .= ' (<a href="Team_Long?Overwrite&amp;ID=' . htmlentities($row['team_id']) . '&amp;Score='
...
HEREDOC;
	...
	while ($row) {
		...
		echo <<<HEREDOC
        <tr>
          <td><a href="Team_Long?ID=$id&amp;Score=$score&amp;xsrf_token={$_SESSION['xsrf_token']}">$team</a></td>
          <td>$school</td>
        </tr>
...
HEREDOC;
	...
	echo <<<HEREDOC
      ...
      <a href="Team_Long">&larr; Cancel</a>
HEREDOC;
	die;
}
...
function do_enter_clarified_score() {
	...
	if (!is_null($row['score_team_long']) && !isSet($_GET['Overwrite'])) {
		if (isSet($_GET['xsrf_token'])) {
			header('Location: Team_Long?ID=' . $_GET['ID'] . '&Score=' . $_GET['Score']);
			die;
		}
		else {
			$msg = 'A score of ' . htmlentities($row['score_team_long'])
				. ' has already been entered for ' . htmlentities($row['name']);
			if ($row['score_team_long'] != $_GET['Score'])
				$msg .= ' (<a href="Team_Long?Overwrite&amp;ID=' . htmlentities($_GET['ID']) . '&amp;Score='
