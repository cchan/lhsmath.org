...
function process_form() {
	...
	$query = 'SELECT COUNT(*) FROM users WHERE LOWER(name)="' . $sql_name . '" AND yog="'
		. mysql_real_escape_string($yog) . '" AND approved!="-1"';
	$result = mysql_query($query) or trigger_error(mysql_error(), E_USER_ERROR);
	$row = mysql_fetch_assoc($result);
	if ($row['COUNT(*)'] != 0) {
	...
	$query = 'INSERT INTO users (name, email, passhash, cell, yog, mailings, approved, email_verification, registration_ip) VALUES ("'
		...
		. mysql_real_escape_string($yog) . '", "'
		. mysql_real_escape_string($mailings) . '", "'
		. mysql_real_escape_string($approved) . '", "'
		. mysql_real_escape_string($verification_code) . '", "'
		. mysql_real_escape_string(htmlentities(strtolower($_SERVER['REMOTE_ADDR']))) . '")';
